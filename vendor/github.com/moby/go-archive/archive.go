...
func createTarFile(root *os.Root, dstPath string, hdr *tar.Header, reader io.Reader, opts *TarOptions) error {
	...
	// syscalls) and never need it. It is only required for mknod and xattrs.
	...
	case tar.TypeSymlink:
		...
		// at creation.
		if err := root.Symlink(hdr.Linkname, dstPath); err != nil {
			return err
		}
...
loop:
	for {
		...
		// If dstPath exits we almost always just want to remove and replace it.
